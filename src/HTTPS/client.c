...
http_request_done(struct evhttp_request *req, void *ctx)
{
...
#else
		while ((oslerr = bufferevent_get_openssl_error(call->bev)))
		{
	...
	call->result = evhttp_request_get_response_code(req);
	strcpy(call->prompt, evhttp_request_get_response_code_line(req));
...
int https_client(char *url, char *data_file, struct _https_callback **call)
{
	int r;
	size_t urisize = strlen(url);
	...
	if ((*call) = (struct _https_callback *)malloc(sizeof(struct _https_callback)))
	{
		memset(*call, 0x00, sizeof(struct _https_callback));
	}
	...
	{
		(*call)->bev = bufferevent_socket_new(base, -1, BEV_OPT_CLOSE_ON_FREE);
	}
	else
	{
#ifndef USE_MBEDTLS
		type = HTTPS;
		(*call)->bev = bufferevent_openssl_socket_new(
#else
...
#else
	bufferevent_openssl_set_allow_dirty_shutdown((*call)->bev, 1);
#endif
	...
	// asynchronous though.
	{
		if (host[0] == '[' && strlen(host) > 2 && ipv6)
		{
			...
			evcon = evhttp_connection_base_bufferevent_new(base, NULL, (*call)->bev,
														   host_ipv6, port);
			free(host_ipv6);
		}
		else
	...
	req = evhttp_request_new(http_request_done, *call);
	if (req == NULL)
	{
