...
int https_client(https_client_t **client, const char *url, int timeout)
{
	if (*client == NULL)
	{
		*client = (https_client_t *)malloc(sizeof(https_client_t));
	}
	...
	(*client)->output_headers = evhttp_request_get_output_headers(req);
	...
	if (data_file)
	{
		...
		(*client)->output_buffer = evhttp_request_get_output_buffer(req);
