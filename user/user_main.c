...
int main(int argc, char *argv[])
{
                                  ...
                                  /*
                                      if (config_allocate(&config))
                                      {
                                          if (config_initializing(config, argc, argv) == OK_SUCCESS)
                                          {
                                              ...
                                              char *dd = mp_malloc(config->pool, 11);
                                              strcpy(dd,"wangyonglin");
                                              config_log_error(config->log,dd);
    ...
    if (system_config_initializing(&config, argc, argv) == ok)
    {
        httpd_initializing(&httpd_config, "0.0.0.0", 80);
    }
